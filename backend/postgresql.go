...
/*
func pairMovieSQLWithTheater(mov Movie, theater string) {
	sqlDelete := `
		DELETE FROM movie WHERE id=$1
	;`
	_, err := db.Exec(sqlDelete, mov.ImdbID[2:])
...
func putTheaterSQL(theater string) {
	sqlInsert := `
		INSERT INTO theater (title)
		VALUES ($1)
	;`

	_, err := db.Exec(sqlInsert, theater)
	if err != nil {
		fmt.Println("Failed to execute SQL Insert:" + err.Error())
	}
...
func getMovieSQLTitleYear(title string, year string) Movie {
	fmt.Println("from database with title: " + title)
	sqlStatement := `
		SELECT title, year, rated, TO_CHAR(released, 'DD Mon YYYY'), genre, director, plot, poster, id
		FROM movie
	...
	rows, err := db.Query(sqlStatement, strings.ToUpper(title), year)
	if err != nil {
		log.Fatal("Failed to execute SQL command:", err)
	}
